...
class SpiderDepartmentSapprft extends PHPCrawler
{
    ...
    static $SeedConf = array(
        "http://www.sapprft.gov.cn/sapprft/govpublic/6649.shtml",
        "http://www.sapprft.gov.cn/sapprft/govpublic/6679.shtml",
    );

    static $ContentHandlers = array(
        "#http://www.sapprft.gov.cn/sapprft/govpublic/[0-9]+([_0-9]+)?\.shtml$# i"   => "void",
        "#http://www.sapprft.gov.cn/sapprft/govpublic/[0-9]+/[0-9]+.shtml# i"  => "handleDetailPage",
    ...
    public function handleListPage(PHPCrawlerDocumentInfo $DocInfo)
    {
        ...
        for ($i = 1; $i <= $pager['total']; $i++) {
            if($i==1){
                $pages[] = $prefix . "6649.shtml";
            }else
                $pages[] = $prefix . "6649_" . $i . "." . "shtml";
        }
...
file_put_contents('spider_' . SpiderDepartmentSapprft::MAGIC . '.pid', $pid);
...
$spider = new SpiderDepartmentSapprft();
$spider->run();

$spider->setFeed(SpiderDepartmentSapprft::$SeedConf[0]);

for ($i=1;$i<count(SpiderDepartmentSapprft::$SeedConf); $i++) {
    $spider->addFeed(SpiderDepartmentSapprft::$SeedConf[$i]);
}
