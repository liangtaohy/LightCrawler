 ...
 * Time: PM4:00
 */
class SpiderFrame2 extends PHPCrawler
{
    ...
    /**
     * SpiderFrame2 constructor.
     * @param array $contentHandlers
     * @param array $linkTextHandlers
     */
    public function __construct(array $contentHandlers = array(), array $linkTextHandlers = array())
    {
        parent::__construct();

        if (!empty($contentHandlers)) {
            $this->ContentHandlers = $contentHandlers;
        }

        if (!empty($linkTextHandlers)) {
            $this->LinkTextHandlers = $linkTextHandlers;
        }
    ...
    {
        $nextPageStr = "function (PHPCrawlerDocumentInfo $DocInfo) { echo \"hahaha\" . PHP_EOL; }";
        $nextPage = eval($nextPageStr);
        $next_page = call_user_func($nextPage, $DocInfo);

        if (!empty($next_page) && is_string($next_page)) {
            return array($next_page);
        }
