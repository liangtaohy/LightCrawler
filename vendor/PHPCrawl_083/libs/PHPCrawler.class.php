...
class PHPCrawler
{
  ...
  /**
   * Limit of retries number
   * @var int The limit in times
   */
  protected $tries_limit = 1;
  ...
  protected function initCrawlerProcess()
  {
    ...
    echo 'url_cache_type: ' . $this->url_cache_type . PHP_EOL;
    // Setup url-cache
    ...
    } else if ($this->url_cache_type == PHPCrawlerUrlCacheTypes::URLCACHE_MYSQL && $this->urlcache_purged == false)
    {
      $this->LinkCache->purgeCache();
      $this->urlcache_purged = true;
    }
    ...
    foreach($this->starting_urls as $url) {
        $url_descriptor = new PHPCrawlerURLDescriptor($url);
        $url_descriptor->url_link_depth = 1;
        $this->LinkCache->addUrl($url_descriptor);
    }
  }

  /**
   * add url into LinkCache
   * @param $url
   */
  ...
  /**
   * @param $working_base_directory
   */
  public function setWorkBaseDir($working_base_directory)
  {
    if (!file_exists($working_base_directory)) {
      mkdir($working_base_directory);
    }
  ...
  { 
    ...
    while($retries>0) {
        ...
        if (!isset($PageInfo->error_occured) || empty($PageInfo->error_occured)) {
            if ((int)($PageInfo->http_status_code) > 400 || (int)($PageInfo->http_status_code) < 100) { // if status code is not valid, let's try again
                // retry again
                continue;
            }
            break;
        } else { // if error occured, let's try again
            continue;
        }
    }
   ...
   * @return bool
   */
  public function addLinktextFollowRule($regex)
  {
    return $this->UrlFilter->addLinkTextFollowRule($regex);
  }
   ...
   * @return bool
   */
  public function addLinktextFilterRule($regex)
  {
    return $this->UrlFilter->addLinkTextFilterRule($regex);
  }
  ...
  /**
   * Retry limit (should be greater or equal to one)
   * @var if $limit <= 0, Retry Limit will be force to 1.
   */
  public function setRetryLimit($limit)
  {
      if ($limit <= 0) $this->tries_limit = 1;
      $this->tries_limit = $limit;
  }
  ...
  {
      $this->PageRequest->setProtocol($protocol);
  }
  ...
  public function setUrlCacheType($url_cache_type)
  {
    ...
    {
      echo 'setUrlCacheType: '. $url_cache_type . PHP_EOL;
