...
class PHPCrawlerURLFilter
{
  ...
  protected $link_text_filter_rules = array();

  protected $link_text_follow_rules = array();
  ...
  { 
    ...
    for ($x=0; $x<count($this->link_text_filter_rules); $x++)
    {
      if (preg_match($this->link_text_filter_rules[$x], $url->linktext)) return false;
    }
    ...
    if (count($this->url_follow_rules) > 0)
    {
      ...
      for ($x=0; $x<count($this->url_follow_rules); $x++)
      {
        ...
        { echo 'regex: matched: ' . $this->url_follow_rules[$x] . ', url:' . $url->url_rebuild . PHP_EOL;
          $match_found = true;
          break;
        }
      }
      ...
      {
        for ($x=0; $x<count($this->link_text_follow_rules); $x++)
        {
          if (preg_match($this->link_text_follow_rules[$x], $url->linktext))
          {
            echo 'regex: matched: ' . $this->link_text_follow_rules[$x] . ', url:' . $url->url_rebuild . ', linktext: ' . $url->linktext . PHP_EOL;
            $match_found = true;
            break;
          }
        }
  ...
  public function addLinkTextFollowRule($regex)
  {
    ...
    if ($check == true)
    {
      $this->link_text_follow_rules[] = trim($regex);
    }
    return $check;
  }
  ...
  public function addLinkTextFilterRule($regex)
  {
    ...
    if ($check == true)
    {
      $this->link_text_filter_rules[] = trim($regex);
    }
    return $check;
  }
}
