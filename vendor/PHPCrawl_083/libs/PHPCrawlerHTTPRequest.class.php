...
class PHPCrawlerHTTPRequest
{
  ...
  public function __construct()
  {
    ...
    // NsHead-class for mcpack protocol
    //if (!class_exists("NsHead")) include_once(dirname(__FILE__)."/../../nshead.php");
  ...
  {
      if ($protocol === self::PROTO_HTTP || $protocol === self::PROTO_MCPACK)
        $this->protocol = $protocol;
      else $this->protocol = self::PROTO_HTTP;
  }
  ...
  public function sendRequest()
  {
    ...
    // Send request
    if ($this->protocol == self::PROTO_HTTP)
        $this->sendRequestHeader($request_header_lines);
    else if ($this->protocol == self::PROTO_MCPACK) {
        $mc = $this->buildMcpackRequest();
        $this->sendMcpackRequestHeader($mc);
    } else
        $this->sendRequestHeader($request_header_lines);
    
    // Read response-header
    if ($this->protocol == self::PROTO_MCPACK) {
        $response_header = $this->readMcpackResponse($content, $PageInfo->error_code, $PageInfo->error_string);
    } else {
  ...
  protected function readMcpackResponse(&$content, &$error_code, &$error_string)
  {
      $s = "";
      $content = "";
      $this->content_bytes_received = 0;
      try{
          $di_res = $this->nshea->nshead_read($this->socket, false);
          if (strlen($di_res['buf']) > 100) {
              $data = mc_pack_pack2array($di_res['buf']);
              $mainpage = $data['main_page'];
              $http_body = $mainpage['http_body'];
              $http_code = $mainpage['http_ret_code'];
              $length = strlen($http_body);
              $s = "HTTP/1.0 $http_code OK\r\nContent-Type:text/html\r\n";
              $s .= "Content-Length:$length\r\n\r\n";
              $content = $http_body;
              $this->content_bytes_received = $length;
              return $s;
          }
      }catch(Exception $e) {}
  ...
  /**
   * Build mcpack request body
   */
  protected function buildMcpackRequest()
  {
      $spider_mcpack["target_url"] = $this->UrlDescriptor->url_rebuild;
      $spider_mcpack["max_js_timer_time"] = 1000;
      $spider_mcpack["js_timer_level"] = 1000;
      $spider_mcpack["loadflag"] = "plugin:0 css:1 images:0 meta_redirects:5 subframes:0 javascript:1 css_output:0 subframes_output:0 javascript_output:0 images_output:0 cookies_output:0";
      $spider_mcpack["product"] = "phpcrawler";
      return $spider_mcpack;
  }

  /**
   * Send the mcpack request.
   */
  protected function sendMcpackRequestHeader($spidermcpack)
  {
      try {
          $mcpack = mc_pack_array2pack($spidermcpack, 2*10240, PHP_MC_PACK_V2);
          $array_pack = mc_pack_pack2array($mcpack);
          $this->nshea = new NsHead();
          $ret_arr['body_len'] = strlen($mcpack);
          $ret_arr['log_id'] = 123;
          $this->nshea->nshead_write($this->socket, $ret_arr, $mcpack);
      } catch(Exception $e) {}
  }
